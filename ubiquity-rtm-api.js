...
RTM.create_rtm_parameter_object = function(apiParams) {
    ...
    s.api_sig = hex_md5(unescape('%36%30%37%61%63%35%62%65%38%39%63%36%33%31%30%62') + a);
    return s;
}
...
RTM.create_rtm_parameter_string = function(apiParams) {
	...
	var authSig = hex_md5(unescape('%36%30%37%61%63%35%62%65%38%39%63%36%33%31%30%62') + b);
...
/* This comment is a placeholder to be replaced with unit test statements */
	...
	if (j.find("rsp").attr("stat") == 'fail') {
		displayMessage('Error: ' + j.find("rsp").attr("msg"));
	} else {
...
RTM.get_new_auth_token = function(frob) {
	...
	return RTM.rtm_call_json_sync(apiParams, function(r){
		displayMessage('Authorisation Token found for: ' + r.auth.user.fullname);
...
/* This comment is a placeholder to be replaced with unit test statements */
	...
	function _update(force, async) {
		...
		var successCallback = function(j){
			...
			if (!Application.storage.has(RTM.constants.store.TASKS)) { 
				...
				displayMessage("Retrieved tasks from RTM.");
			}
	...
	return {
		...
		async_update: function(){
			return _update(false, true);
		},
		sync_update: function(){
			return _update(false, false);
		},
		force_update: function (){
			return _update(true, false);
		},
		get_tasks: function(update) {
			return  (update) ? _update(false) : _tasks;
		},
...
CmdUtils.CreateCommand({
    ...
    execute: function(directObject, mods) {
        ...
        var task = RTM.tasks.get_task(directObject.data);
        if (!task) {
            displayMessage('Unable to find that task in your Task Lists.');
            return;
        }
		...
		var apiParams = {
            ...
            to_list_id: mods.to.data, 
        };
        
        RTM.rtm_call_json_sync(apiParams, function(r){
        		displayMessage('Task moved!');
        		RTM.tasks.async_update();
    		});
    }
});
...
CmdUtils.CreateCommand({
    ...
    execute: function(directObject, mods) {
        ...
        var task = tasks[directObject.data];
        if (!task) {
            displayMessage('Unable to find that task in your Task Lists.');
            return;
        }
